...
results_folder = f"Results_SOMPY/SOMPY_{timestamp}"
...
class UMatrixViewNew(sompy.umatrix.UMatrixView):
    def Get_label_Umatrix(self, class_labels, coord):
        ...
        # unique_labels = np.unique(label_matrix[label_matrix != -1])  # Exclude -1
        # cluster_colors = plt.cm.tab10(np.linspace(0, 1, len(unique_labels)))
        # cmap = ListedColormap(np.vstack(([[0.5, 0.5, 0.5, 0.5]], cluster_colors)))  # Gray for -1

        # plt.figure(figsize=(10, 7))
        # plt.imshow(label_matrix, cmap=cmap, interpolation='nearest')
        # plt.colorbar(ticks=np.append(-1, unique_labels), label='Cluster Label')
        # plt.title('Cluster Label Matrix')
    ...
    def show(self, som, distance2=1, row_normalized=False, show_data=False,
        ...
        if show_class == True and class_labels is not None:
            ...
            #print umat_class
            #im = ax.imshow(umat_class,interpolation='nearest', cmap=plt.cm.get_cmap('gist_ncar'), alpha=1)
            ...
            # im = ax.imshow(umat, interpolation='nearest', cmap=plt.cm.get_cmap(color_map), alpha=1)
...
class SOM_Tool():
    ...
    def Draw_SOM_RAW(self, fig=None, ax=None, contooor = False, blob = False, show_data = False, show_troj = False, troj_range = None, show_mag = True, color_map='jet'):      
        self.fig = fig
        self.fig.clf()
        self.ax = self.fig.add_subplot(111)
        visual = UMatrixViewNew(1,1,"",col_size=200)
        visual.set_fig_ax(self.fig, self.ax)
        visual.show(self.som,contooor=contooor,blob=blob,show_data=show_data,show_troj=show_troj,troj_range=troj_range, show_mag =show_mag, color_map="jet")
        _, _, coord = visual.show(self.som, contooor=contooor, blob=blob, show_data=show_data,
...
def Draw_SOM_Clusters(self, class_labels, fig=None, ax=None, show_mag=True, color_map='tab10'):
        ...
        visual = UMatrixViewNew(1, 1, "", col_size=200)
        visual.set_fig_ax(self.fig, self.ax)
        _, _, coord = visual.show(self.som, show_class=True, class_labels=class_labels, show_mag=show_mag, color_map=color_map)
        return coord
...
# ---------------------- ADD THIS EXECUTION SECTION ----------------------
if __name__ == "__main__":
    # Use the scaled data for SOM
    nrm_df = data_scaled
    
    # Create and save the raw SOM
    som_class = SOM_Tool(nrm_df, mapsize=(som_l, som_w))
    som_class.Do_SOM()
    DR_df = som_class.SOM_DR()
    print("%d x %d was used to generate SOMPY" % (som_l, som_w))
    
    # Create the visualization and save it
    fig_som = plt.figure(figsize=(10, 7))
    som_class.Draw_SOM_RAW(fig=fig_som, show_mag=True)
    plt.tight_layout()
    
    # Save to the results folder
    output_path = os.path.join(results_folder, "raw_som.png")
    plt.savefig(output_path)
    print(f"SOM visualization saved to {output_path}")
