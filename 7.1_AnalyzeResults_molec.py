...
print(f"Looking for data files in folder: {folder}")
...
# If original_data_SMILES.csv doesn't exist, try other locations
...
def update_scatter_plot(color_option):
    if color_option == 'cluster':
        ...
        # Update the color axis to show the label names properly
        fig.update_layout(
            coloraxis_colorbar=dict(
                title="Cluster Label",
                tickvals=list(range(len(reduced_data_copy['Label'].unique()))),
                ticktext=sorted(reduced_data_copy['Label'].unique())
            )
        )
...
def display_hover_data(hoverData):
    if hoverData and 'points' in hoverData and hoverData['points']:
        ...
        if 'customdata' in point_info:
            if isinstance(point_info['customdata'], list):
                point_index = point_info['customdata'][0] - 1  # Convert to 0-based index
            else:
                point_index = point_info['customdata'][0] - 1
