...
def create_dash_app(folder_path="RESULTS/2025-03-09_20-03-29-logcmc", port=8050):
    ...
    # app.layout = html.Div(style={'fontFamily': 'Arial, sans-serif', 'margin': '20px', 'maxWidth': '1000px', 'marginLeft': 'auto', 'marginRight': 'auto'}, children=[
    ...
    app.layout = html.Div(style={
        ...
        html.Div([
            html.H3("Selected Molecule Structures", style={'marginTop': '30px', 'marginBottom': '15px'}),
    ...
    def update_scatter_plot(color_option):
        if color_option == 'cluster':
            # Get cluster labels as numerical values for continuous color scale
            # First, check what columns are available in cluster_labels
            print(f"Available columns in cluster_labels: {cluster_labels.columns.tolist()}")
            
            # Create a copy of reduced_data for merging
            ...
            if 'Label' in cluster_labels.columns:
                ...
                if len(potential_columns) > 0:
                    ...
                    print(f"Using '{cluster_column}' as cluster column instead of 'Label'")
                else:
                    # Fallback: create a dummy column if no suitable column is found
                    cluster_column = 'Cluster'
                    cluster_labels[cluster_column] = 0
                    print("No cluster column found, using dummy values")
        ...
        fig.update_layout(
            ...
            # Set a more square aspect ratio with fixed dimensions
            width=1000,  # Reduce width
            height=600,  # Maintain good height for visibility
